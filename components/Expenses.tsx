...
const ExpenseForm: React.FC<{ onFormSubmit: () => void }> = ({
  onFormSubmit,
}) => {
  const {
    addExpense,
  ...
  const [formData, setFormData] = useState<
    Omit<Expense, "id" | "status" | "submittedBy">
  >({
    vendor: "",
    invoiceNumber: "",
  ...
  useEffect(() => {
    ...
    if (Math.abs(newTotal - formData.total) > 0.01) {
      setFormData((prev) => ({ ...prev, total: Number(newTotal.toFixed(2)) }));
    }
  }, [formData.lineItems, formData.tax, formData.total]);

  // Real-time Budget Validation
  useEffect(() => {
    if (!formData.responsibilityId || formData.total <= 0) {
      setBudgetWarning(null);
      return;
    }

    const resp = responsibilities.find(
      (r) => r.id === formData.responsibilityId
    );
    if (!resp) return;
    ...
    const currentSpent = expenses
      .filter(
        (e) =>
          e.responsibilityId === formData.responsibilityId &&
          e.status !== ExpenseStatus.REJECTED &&
          e.isDeleted !== true
      )
      .reduce((sum, e) => sum + e.total, 0);

    const projectedTotal = currentSpent + formData.total;

    if (projectedTotal > resp.budget) {
      if (currentUser.role === Role.FINANCE_MANAGER) {
        setBudgetWarning({
          message: t("expenses.budgetWarning.manager", {
            over: (projectedTotal - resp.budget).toFixed(2),
          }),
          type: "warning",
        });
      } else {
        setBudgetWarning({
          message: t("expenses.budgetWarning.user"),
          type: "error",
        });
      }
    } else {
  ...
  const updateFormWithExtractedData = (data: ExtractedInvoiceData) => {
    setFormData((prev) => ({
      ...prev,
      vendor: data.vendorName || prev.vendor,
      ...
      lineItems:
        data.lineItems && data.lineItems.length > 0
          ? data.lineItems
          : prev.lineItems,
      tax: data.taxes || prev.tax,
      total: data.totalAmount || prev.total,
    }));
    addToast(t("expenses.processingInvoice") + " Complete", "success");
  };
  ...
  const processInvoice = async (file: File) => {
    ...
    try {
      // 1. Convert to Base64 immediately
      ...
      // 2. Add to attachments list immediately
      setFormData((prev) => ({
        ...prev,
        attachments: [...(prev.attachments || []), newAttachment],
      }));

      // 3. Optimizations for OCR
      if (mimeType.startsWith("image/")) {
        try {
          base64 = await preprocessImage(file);
        } catch (e) {
          console.warn("Image preprocessing failed, using original file.", e);
        }
      }
      ...
      if (extractedData) {
        ...
        addToast(t("expenses.error.extract"), "error");
      }
    } catch (err) {
      console.error(err);
      setError(t("expenses.error.processing"));
      addToast(t("expenses.error.processing"), "error");
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (budgetWarning?.type === "error") {
      setError(budgetWarning.message);
      return;
    }
  ...
  return (
    <Card>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex flex-col space-y-2">
          <div className="flex space-x-2">
            <Button type="button" onClick={() => fileInputRef.current?.click()}>
              {t("expenses.uploadInvoice")}
            </Button>
            <Button type="button" onClick={startCamera}>
              {t("expenses.scanWithCamera")}
            </Button>
          </div>
          <p className="text-xs text-gray-500">
            Supports JPG, PNG, PDF (Multiple allowed)
          </p>
        </div>
        <input
          type="file"
        ...
        {isProcessing && (
          <div className="flex items-center space-x-2 text-text-dim">
            <Spinner /> <span>{t("expenses.processingInvoice")}</span>
          </div>
        )}
        ...
        {formData.attachments && formData.attachments.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {formData.attachments.map((att, idx) => (
              <div
                key={idx}
                className="bg-blue-50 p-3 rounded-lg border border-blue-200 flex justify-between items-center"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Input
            ...
            onChange={handleChange}
            placeholder={t("expenses.invoiceNumber")}
          />
          <Input
            name="date"
            type="date"
        ...
        <div>
          ...
          <Button
            type="button"
            onClick={addLineItem}
            className="text-sm !bg-gray-200 !text-black"
          >
            {t("expenses.addLineItem")}
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Input
            ...
            onChange={handleChange}
            placeholder={t("expenses.totalAmount")}
            required
            readOnly
        ...
        <Button
          type="submit"
          className="w-full"
          disabled={isProcessing || budgetWarning?.type === "error"}
        >
      ...
      <Modal
        isOpen={isCameraOpen}
        onClose={stopCamera}
        title={t("expenses.scanInvoiceTitle")}
      >
...
const ExpenseList: React.FC = () => {
  const { expenses, currentUser, updateExpenseStatus, users, vendors, t } =
    useAppContext();
  ...
  const relevantExpenses = isManager
    ? expenses
  ...
  const filteredExpenses = useMemo(() => {
    let filtered = relevantExpenses.filter((exp) => {
      ...
      const matchesVendor =
        filterVendor === "" ||
        exp.vendor.toLowerCase().includes(filterVendor.toLowerCase());
      const matchesStatus =
    ...
    filtered.sort((a, b) => {
      const valA =
        sortConfig.key === "date" ? a.createdAt || a.date : a[sortConfig.key];
      const valB =
        sortConfig.key === "date" ? b.createdAt || b.date : b[sortConfig.key];
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th
                ...
                className="px-6 py-3 text-start text-xs font-medium text-text-dim uppercase tracking-wider cursor-pointer hover:text-text-main"
              >
                {t("expenses.invoiceNumber")} {renderSortIcon("invoiceNumber")}
              </th>
              <th
                onClick={() => requestSort("date")}
                className="px-6 py-3 text-start text-xs font-medium text-text-dim uppercase tracking-wider cursor-pointer hover:text-text-main"
              >
                {t("expenses.date")} {renderSortIcon("date")}
              </th>
              <th
                onClick={() => requestSort("vendor")}
                className="px-6 py-3 text-start text-xs font-medium text-text-dim uppercase tracking-wider cursor-pointer hover:text-text-main"
              >
                {t("expenses.vendor")} {renderSortIcon("vendor")}
              </th>
              <th
                onClick={() => requestSort("category")}
                className="px-6 py-3 text-start text-xs font-medium text-text-dim uppercase tracking-wider cursor-pointer hover:text-text-main"
              >
                {t("expenses.category")} {renderSortIcon("category")}
              </th>
              <th className="px-6 py-3 text-start text-xs font-medium text-text-dim uppercase tracking-wider">
              ...
              <th
                ...
                className="px-6 py-3 text-start text-xs font-medium text-text-dim uppercase tracking-wider cursor-pointer hover:text-text-main"
              >
                {t("expenses.total")} {renderSortIcon("total")}
              </th>
              <th className="px-6 py-3 text-start text-xs font-medium text-text-dim uppercase tracking-wider">
              ...
              <th
                ...
                className="px-6 py-3 text-start text-xs font-medium text-text-dim uppercase tracking-wider cursor-pointer hover:text-text-main"
              >
                {t("expenses.status")} {renderSortIcon("status")}
              </th>
              <th className="px-6 py-3 text-start text-xs font-medium text-text-dim uppercase tracking-wider">
                {t("expenses.actions")}
              </th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredExpenses.length > 0 ? (
              filteredExpenses.map((exp) => {
                const submitter =
                  users.find((u) => u.id === exp.submittedBy)?.name ||
                  t("expenses.unknown");
                const statusColor = {
                ...
                // Handle backward compatibility where attachments might not be an array
                const attachCount = exp.attachments
                      ...
                      {isManager && exp.status === ExpenseStatus.PENDING && (
                        <>
                          <Button
                            ...
                            className="!bg-secondary !text-white text-xs px-2 py-1"
                          >
                            {t("expenses.approve")}
                          </Button>
                          <Button
              ...
              <tr>
                <td
                  colSpan={9}
                  className="px-6 py-4 text-center text-sm text-text-dim"
                >
                  {t("filters.noResults")}
                </td>
              </tr>
          ...
          <tfoot className="bg-gray-100 font-semibold">
            <tr>
              <td colSpan={5} className="px-6 py-3 text-end text-sm">
                {t("filters.total")}
              </td>
              ...
              <td colSpan={3} className="px-6 py-3 text-sm">
                <span className="text-text-dim text-xs font-normal">
                  ({t("expenses.tax")}: ${Number(totals.tax).toFixed(2)})
                </span>
              </td>
            </tr>
          </tfoot>
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => setIsFormVisible((prev) => !prev)}>
          {isFormVisible ? t("expenses.closeForm") : t("expenses.newExpense")}
        </Button>
      </div>
      {isFormVisible && (
        <ExpenseForm onFormSubmit={() => setIsFormVisible(false)} />
      )}
      <ExpenseList />
    </div>
  );
