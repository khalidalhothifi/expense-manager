...
import { 
    User, Role, Group, FinancialResponsibility, Expense, ExpenseStatus, NotificationSettings, 
    UserNotificationPreferences, NotificationTrigger, Vendor
} from '../types';
import { INITIAL_NOTIFICATION_SETTINGS, INITIAL_USER_PREFERENCES } from '../constants';
...
interface AppContextType {
    ...
    updateUser: (userId: string, userData: Partial<Omit<User, 'id' | 'password'>>, groupIds?: string[]) => Promise<void>;
    ...
    expenses: Expense[];
    addExpense: (expense: Omit<Expense, 'id' | 'status' | 'submittedBy'>) => Promise<void>;
    updateExpenseStatus: (expenseId: string, status: ExpenseStatus) => Promise<void>;
    addResponsibility: (resp: Omit<FinancialResponsibility, 'id' | 'history'>) => Promise<void>;
    updateResponsibility: (id: string, data: Partial<Omit<FinancialResponsibility, 'id' | 'history'>>) => Promise<void>;
    reallocateBudget: (fromId: string, toId: string, amount: number) => Promise<boolean>;
    notificationSettings: NotificationSettings;
    setNotificationSettings: React.Dispatch<React.SetStateAction<NotificationSettings>>;
    saveNotificationSettings: (settings: NotificationSettings) => Promise<void>;
    userNotificationPreferences: UserNotificationPreferences;
    setUserNotificationPreferences: React.Dispatch<React.SetStateAction<UserNotificationPreferences>>;
    sendNotification: (trigger: NotificationTrigger, details: Record<string, string | number>) => void;
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const [userNotificationPreferences, setUserNotificationPreferences] = useState<UserNotificationPreferences>(INITIAL_USER_PREFERENCES);
    ...
    const fetchData = useCallback(async () => {
        if (currentUser && translations) { 
            setIsLoading(true);
            const [fetchedUsers, fetchedGroups, fetchedVendors, fetchedResponsibilities, fetchedExpenses, fetchedSettings] = await Promise.all([
                api.getUsers(),
                api.getGroups(),
                api.getVendors(),
                api.getResponsibilities(),
                api.getExpenses(),
                api.getNotificationSettings ? api.getNotificationSettings() : Promise.resolve(INITIAL_NOTIFICATION_SETTINGS)
            ]);
    ...
    const updateUser = async (userId: string, userData: Partial<Omit<User, 'id' | 'password'>>, groupIds?: string[]) => {
    ...
    const sendNotification = useCallback(async (trigger: NotificationTrigger, details: Record<string, string | number>) => {
        ...
        if (currentUser) {
           await api.sendEmail(currentUser.email, subject, body);
        }

        const toastMessage = details._toastMessage ? String(details._toastMessage) : `[Email Sent] Subject: ${subject}`;
        addToast(toastMessage, 'success');

    }, [notificationSettings.templates, addToast, currentUser, api, language]);

    const addExpense = async (expenseData: Omit<Expense, 'id' | 'status' | 'submittedBy'>) => {
        if (!currentUser) return;

        const existingVendor = vendors.find(v => v.name.toLowerCase() === expenseData.vendor.toLowerCase());
        if (!existingVendor) {
    ...
    const updateExpenseStatus = async (expenseId: string, status: ExpenseStatus) => {
        ...
        if (updatedExpense) {
            ...
            const trigger = status === ExpenseStatus.APPROVED ? NotificationTrigger.EXPENSE_APPROVED : NotificationTrigger.EXPENSE_REJECTED;
            sendNotification(trigger, { vendor: updatedExpense.vendor, total: updatedExpense.total });
            addToast(`Expense ${status.toLowerCase()}`, 'success');
        }
    };
    
    const addResponsibility = async (respData: Omit<FinancialResponsibility, 'id' | 'history'>) => {
        if (!currentUser) return;
        const newResp = await api.addResponsibility(respData, currentUser.name);
        setResponsibilities(prev => [...prev, newResp]);
        sendNotification(NotificationTrigger.RESPONSIBILITY_ASSIGNED, {
    ...
    const updateResponsibility = async (id: string, respData: Partial<Omit<FinancialResponsibility, 'id' | 'history'>>) => {
        if (!currentUser) return;
        const updated = await api.updateResponsibility(id, { ...respData, editorName: currentUser.name });
        if (updated) {
    ...
    const reallocateBudget = async (fromId: string, toId: string, amount: number) => {
        if (!currentUser) return false;
        const result = await api.reallocateBudget(fromId, toId, amount, currentUser.name);
        if (result.success) {
            setResponsibilities(prev => prev.map(r => {
                const updated = result.updatedResponsibilities.find(ur => ur.id === r.id);
